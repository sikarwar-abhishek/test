...
function CommentsSection({ postId, onBack }) {
  ...
  return (
    <div className="w-full sm:w-96 bg-white rounded-xl shadow-sm border h-full sm:h-fit sm:max-h-[calc(100vh-8rem)] flex flex-col">
      {/* Header */}
      <div className="p-3 sm:p-4 relative border-b border-gray-100 w-full flex items-center">
        <button
          onClick={onBack}
          type="button"
          className="p-1 sm:p-2 hover:bg-gray-100 rounded-full transition-colors flex-shrink-0 z-10"
        >
          <ArrowLeft size={18} className="text-gray-600 sm:w-5 sm:h-5" />
        </button>
        <h2 className="text-lg sm:text-xl flex-1 text-center font-medium font-poppins -ml-10 sm:-ml-12">
          Comments
        </h2>
      </div>
              ...
              <div key={comment.id} className="space-y-2 sm:space-y-3">
                ...
                <div className="flex gap-2 sm:gap-3">
                  ...
                  <div className="flex-1 overflow-hidden">
                    <div className="bg-gray-100 rounded-2xl p-2 sm:p-3 relative overflow-hidden">
                      {/* 3-dot menu for user's own comments */}
                      {userProfile?.data?.id === comment.comment_by && !editingCommentId &&  (
                        <div className="absolute top-1 right-1 sm:top-2 sm:right-2 dropdown-container">
                          <button
                          ...
                          >
                            <MoreVertical
                              size={12}
                              className="text-gray-500 sm:w-3.5 sm:h-3.5"
                            />
                          </button>

                          {/* Dropdown menu */}
                          {showDropdown === comment.id && (
                            <div className="absolute sm:right-2 right-4 top-0 sm:top-4 bg-white border rounded-lg shadow-lg py-1 z-10 min-w-[80px] sm:min-w-[100px]">
                              <button
                                onClick={() => handleEditComment(comment)}
                                className="w-full px-2 sm:px-3 py-1 text-left text-xs sm:text-sm hover:bg-gray-100 flex items-center gap-1 sm:gap-2"
                              >
                      ...
                      <h4 className="font-semibold font-poppins text-gray-900 mb-1 text-xs sm:text-sm pr-4 sm:pr-6 line-clamp-1">
                      ...
                      {editingCommentId === comment.id ? (
                        <div className="space-y-2">
                          <textarea
                            ...
                            className="w-full p-2 border rounded-lg text-xs sm:text-sm resize-none focus:outline-none focus:ring-2 focus:ring-blue-500"
                            rows={2}
                          ...
                          <div className="flex gap-1 sm:gap-2">
                            <button
                              ...
                              className="px-2 sm:px-3 py-1 bg-blue-500 text-white rounded text-xs hover:bg-blue-600 disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-1"
                            >
                              {editCommentMutation.isPending ? (
                            ...
                            <button
                              ...
                              disabled={editCommentMutation.isPending}
                              className="px-2 sm:px-3 py-1 bg-gray-500 text-white rounded text-xs hover:bg-gray-600 disabled:opacity-50 flex items-center gap-1"
                            >
                        ...
                        <p className="text-gray-700 font-poppins leading-relaxed text-xs sm:text-sm pr-4 sm:pr-6">
      ...
      <div className="p-3 sm:p-4 border-t border-gray-100">
        <div className="flex items-center gap-0 border rounded-full p-1.5 sm:p-2">
          ...
          <div className="flex-1 flex items-center gap-1 sm:gap-2">
            <input
              ...
              className="flex-1 px-2 sm:px-3 py-1.5 sm:py-2 rounded-full outline-none font-poppins placeholder-gray-400 transition-all text-xs sm:text-sm disabled:opacity-50 disabled:cursor-not-allowed"
            />
            {commentMutation.isPending && (
        ...
        {commentMutation.isError && (
          <div className="mt-2 text-xs text-red-500 px-2 sm:px-3">
